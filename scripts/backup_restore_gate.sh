...
cd /tmp/tbv_gate_a
PORT=7860 TBV_SEED=12345 python3 scripts/supervisor.py > sup_a.log 2>&1 &
SUP_A=$!
sleep 8
cat sup_a.log
...
echo "--- POST /backup ---"
curl -s -X POST http://localhost:7860/backup; echo
...
if [ -n "$GITHUB_TOKEN" ]; then
    ...
    cd /tmp/tbv_gate_b
    PORT=7861 TBV_SEED=12345 GITHUB_TOKEN="$GITHUB_TOKEN" TBV_DATA_REPO="${TBV_DATA_REPO:-DevOGhere/10-Bit-Village-Data}" \
